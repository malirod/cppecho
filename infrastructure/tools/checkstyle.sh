...
if [ -n "$CPP_FILES" ]; then
    infrastructure/tools/cppcheck/cppcheck -j $(nproc) --error-exitcode=1 --language=c++ --std=c++11 --std=posix --platform=unix64 --enable=warning,style,performance,portability,information,missingInclude --template='{id}:{file}:{line} {message}' --suppressions-list=infrastructure/tools/cppcheck/suppressions --inline-suppr --inconclusive -Isrc $CPP_FILES

...
CPP_FILES=$(find flatasync cppecho testrunner -type f \( -iname "*.h" -o -iname "*.cc" -o -iname "*.cpp" -o -iname "*.c" -o -iname "*.hpp" \))
...
if [ -n "$CPP_FILES" ]; then
    tools/cppcheck/cppcheck -j $(nproc) --error-exitcode=1 --std=c++11 --language=c++ --std=posix --platform=unix64 --enable=warning,style,performance,portability,information,missingInclude --template='{id}:{file}:{line} {message}' --suppressions-list=tools/cppcheck/suppressions --inline-suppr --inconclusive -Icppecho/src -Iflatasync/src $CPP_FILES
